...
const layoutType = computed(() =>
    theme.value?.cover?.twoColumns ? 'twoColumns' : theme.value?.cover?.showCover?.coverLayout ?? 'left'
)
...
// 判断是否显示封面
const showCover = () => theme.value?.cover?.showCover?.enable

// 获取封面图片 按优先级获取：cover > defaultCover > false
const getCover = ({ cover: itemCover }) => {
  const { cover } = theme.value ?? {}
